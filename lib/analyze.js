...
export const ANALYSIS_SCHEMA = {
  ...
  required: [
    ...
    'repurpose_ideas', 'identifiers', 'match_precision', 'shipping_class',
  ],
  properties: {
    ...
    identifiers: {
      type: 'object',
      additionalProperties: false,
      required: ['model', 'upc', 'asin', 'epid', 'discogs_release_id', 'pricecharting_id'],
      properties: {
        model: { type: 'string', description: 'Visible model number or catalog number, empty string if not visible' },
        upc: { type: 'string', description: 'Visible UPC/EAN barcode digits, empty string if not visible' },
        asin: { type: 'string', description: 'Amazon ASIN if visible or unmistakable, empty string otherwise' },
        epid: { type: 'string', description: 'eBay product ID if visible or unmistakable, empty string otherwise' },
        discogs_release_id: { type: 'string', description: 'Discogs release ID for clearly identified music media, empty string otherwise' },
        pricecharting_id: { type: 'string', description: 'PriceCharting product ID for clearly identified games/cards/comics, empty string otherwise' },
      },
    },
    match_precision: { type: 'string', enum: ['exact', 'similar', 'category'], description: 'How precisely the visible information identifies the item' },
    shipping_class: { type: 'string', enum: ['small', 'medium', 'large', 'bulky-freight', 'unknown'], description: 'Shipping burden for routing decisions' },
...
- Extract exact identifiers when visible: model number, UPC/EAN, ASIN, eBay product ID, Discogs release ID, or PriceCharting product ID. Leave unknown identifiers as empty strings. Set match_precision to "exact" only when a visible identifier or unambiguous brand/model is present, "similar" for same product line with uncertain variant, and "category" for generic identification.
- Set shipping_class to small, medium, large, bulky-freight, or unknown based on the photographed item as a normal parcel.
